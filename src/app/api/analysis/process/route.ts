...
export async function POST(request: Request) {
  try {
    const { session, error } = await requirePermission("RUN_ANALYSIS");
    ...
    const { acquireAnalysisLock, releaseAnalysisLock } = await import('@/services/analysisLock');
    ...
    try {
      ...
      try { await import('@/services/analysisLock').then(m => m.releaseAnalysisLock(String(questionnaire._id))); } catch (e) { }
    }
